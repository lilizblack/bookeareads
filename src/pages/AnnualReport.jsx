...
import { BookOpen, Calendar, TrendingUp, Award, Clock, PauseCircle, XCircle, AlertTriangle, Trophy, Star, Library, DollarSign, Heart } from 'lucide-react';
...
const AnnualReport = () => {
    ...
    return (
        <div className="pb-24 pt-4">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-6">Annual Report</h1>
            ...
            <div className="bg-white dark:bg-slate-900 p-3 sm:p-6 rounded-2xl border border-slate-100 dark:border-slate-800 mb-6 overflow-hidden">
                    ...
                    {monthlyData.map((data, i) => {
                        ...
                        return (
                            ...
                            >
                                {/* Tooltip */}
                                {isHovered && (
                                    <div className="absolute -top-16 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-3 py-2 rounded-lg shadow-lg z-10 whitespace-nowrap animate-fade-in pointer-events-none">
                                        ...
                                        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-0 h-0 border-l-4 border-l-transparent border-r-4 border-r-transparent border-t-4 border-t-slate-900 dark:border-t-white"></div>
                                    </div>
                                )}

                                {/* Bar */}
                                <div
                                    className={`w-full rounded-t sm:rounded-t-lg cursor-pointer transition-all duration-500 ${isHovered
                                        ? 'bg-red-600'
                                        : 'bg-red-500'
                                        }`}
                                    style={{
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-3xl font-black text-slate-900 dark:text-white mt-1">{stats.readThisYear * 300}</p> {/* Mock pages calculation */}
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
            ...
            {stats.worstBook && stats.worstBook.rating <= 2 && (
                <div
                    ...
                    className="bg-red-50 dark:bg-red-900/10 p-6 rounded-2xl border border-red-100 dark:border-red-900/30 flex items-center justify-between shadow-sm cursor-pointer hover:shadow-md transition-shadow group col-span-full md:col-span-1"
                >
            ...
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-700 col-span-full md:col-span-2 lg:col-span-4">
            ...
            {/* Library Growth Card */}
            <div className="bg-slate-900 dark:bg-slate-800 p-6 rounded-2xl border border-slate-700 dark:border-slate-700 col-span-full text-white shadow-xl shadow-slate-200 dark:shadow-none relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6">
                <ChartCard title="Top Genres" data={topGenres} colors={['#8B5CF6', '#EC4899', '#3B82F6', '#10B981', '#F59E0B']} />
                <ChartCard title="Book Formats" data={formatData} colors={['#6366F1', '#F43F5E', '#84CC16', '#06B6D4', '#D946EF']} />
            </div>
        </div>
    );
};
